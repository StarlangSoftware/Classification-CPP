...
private:
    ...
    double gamma;
    double coefficient0;
    [[nodiscard]] double linear(int i, int j) const;
    [[nodiscard]] double polynom(int i, int j) const;
    [[nodiscard]] double rbf(int i, int j) const;
    [[nodiscard]] double sigmoid(int i, int j) const;
public:
    Kernel(int l, const vector<NodeList>& x, KernelType kernelType, int degree, double gamma, double coefficient0);
    Kernel();
    void swapIndex(int i, int j);
    [[nodiscard]] double function(int i, int j) const;
